...
class TestPickingException(TransactionCase):
    ...
    def setUp(self):
        super(TestPickingException, self).setUp()
        # Useful models
        self.StockPicking = self.env['stock.picking']
        self.StockPickingLine = self.env['stock.move']
        ...
        self.picking_exception_confirm = \
            self.env['picking.exception.confirm']
        self.exception_noemail = self.env.ref(
            'picking_exception.po_excep_no_email')
        self.exception_qtycheck = self.env.ref(
            'picking_exception.pol_excep_qty_check')
        self.po_vals = {
    ...
    def test_stock.picking_exception(self):
        ...
        self.po = self.StockPicking.create(self.po_vals.copy())

        # confirm quotation
        self.po.button_validate()
        self.assertEqual(self.po.state, 'draft')
        # test all draft po
        self.po2 = self.StockPicking.create(self.po_vals.copy())
        ...
        self.po.button_validate()
        self.assertEqual(self.po.state, 'picking')

        # Add a picking line to test after PO is confirmed
        # set ignore_exception = False  (Done by onchange of picking_line)
        field_onchange = self.StockPicking._onchange_spec()
        self.assertEqual(field_onchange.get('picking_line'), '1')
        self.env.cache.invalidate()
        self.po3New = self.StockPicking.new(self.po_vals.copy())
        self.po3New.ignore_exception = True
        self.po3New.state = 'picking'
        ...
        # Simulation the opening of the wizard picking_exception_confirm and
        # set ignore_exception to True
        po_except_confirm = self.picking_exception_confirm.with_context(
            {
