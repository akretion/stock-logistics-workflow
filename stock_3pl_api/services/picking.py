...
class PickingService(Component):
    ...
    def search(self, picking_search_param):
        ...
        if picking_search_param.picking_type_id:
            domain = expression.AND(
                [
                    domain,
                    [("picking_type_id", "=", picking_search_param.picking_type_id)],
                ]
            )
                ...
                [
                    ...
                    [
                        (
                            "picking_type_id",
                            "ilike",
                            picking_search_param.picking_type_name,
                        )
                    ],
                ]
            )
    ...
    def update(self, _id, picking_update_param):
        ...
        if picking_update_param.moves:
            for move_update_param in picking_update_param.moves:
                if move_update_param.id not in picking.move_ids_without_package.mapped(
                    "id"
                ):
    ...
    def _to_move_info(self, move):
        ...
        move_info.lines = [self._to_move_line_info(line) for line in move.move_line_ids]
        return move_info
